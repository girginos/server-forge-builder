...
export default function ServerCard({ id, name, image, formFactor, cpu, maxRam, price, oldPrice, badge }: ServerCardProps) {
  ...
  return (
    <div className="group bg-card rounded-lg border shadow-card hover:shadow-glow transition-all duration-300 overflow-hidden">
      {badge && (
        <div className="px-4 pt-3">
          <Badge className="bg-sale text-primary-foreground text-[10px]">{badge}</Badge>
        </div>
      )}
      <div className="p-4 flex items-center justify-center h-48 bg-muted/30">
        <img src={image} alt={name} className="max-h-40 object-contain group-hover:scale-105 transition-transform duration-300" />
      </div>
